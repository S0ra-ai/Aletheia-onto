...
>
> ✅ **已完成：** `platform_db` 签名已全部改为 `PlatformDb`（上下文／路径／字符串）
...
> ⚠️ **实现方式已变。** 原为「声明完所有路由后，反射 `app.routes` 逐条复制到带前缀的
> 路径」。FastAPI 0.141 起 `include_router` 改为**延迟展开**——`app.routes` 里存的是
> 占位对象而非路由，反射因此看不到任何经 router 注册的端点，**静默**不生成其 `/v1` 副本。
> 静默正是危险所在：裸路径照常工作，直到某个版本化调用方收到 404 才会暴露。
>
> 现改为在注册时就显式双注册（`include_router_twice`），不依赖任何反射。
> 同时 `declared_routes()` 成为「本 API 暴露了什么」的唯一受支持入口——
> 否则测试只能去读 `_IncludedRouter` 这类私有结构，而它已经变过一次。
>
> 相应地，`test_api_versioning.py` 的断言从「比对声明列表」改为**发真实请求**：
> 受保护端点在裸路径与 `/v1` 上都必须返回 401。列表比对曾在 `/v1` 实际不可达时通过。

🔶 **部分完成：拆 `APIRouter`。**
工作流／权限／工具三组（23 个端点）已移入 `routers/workflow_permission_routes.py`，
`api.py` 从 2548 行降至 2201 行，模块中段的 import 块已消除，
`ruff` 的 `E402` 例外随之删除。

共享运行时（`AUTH_ENABLED`、开发身份、`current_principal`）下沉到
`http_runtime.py`。这是必须的：router 若从 `api.py` 取这些符号就会成环，
而惯用的绕法——函数内 import——恰是 `test_module_boundaries.py` 要拦的模式。
`AUTH_ENABLED` 尤其不能每个 router 各拷一份：它在 import 期求值一次，
拷贝会让某个 router 与中间件对「认证是否开启」产生分歧，而分歧的失败方向是免鉴权。

🚧 仍有 113 个端点在 `api.py` 内，按关注点继续外移。
