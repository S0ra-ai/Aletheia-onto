...
**更新 README 时应附带效果图。** 一个看不到样子的框架，读者无法判断它是否值得试。

规矩：

- **必须是真实运行截图**，不是设计稿、示意图或手绘 mockup
- **数据必须来自合成示例**（`examples/` 或内置样例），
  绝不能出现真实客户名称、真实连接串、真实业务数据
- 放在 `docs/images/`，**文件名用 ASCII**（中文名在 Markdown 里需 URL 编码，
  且会给部分工具链带来麻烦）
- 宽度压到 1600px 以内，单张控制在 ~350KB 以内（`sips --resampleWidth 1600`）
- **改动了界面就要重截**。过时的截图和过时的文字一样有害
- **如果图里出现了「有存储但未生效」的字段，必须在图旁边标注**。
  一张展示了不生效的输入框而不加说明的截图，等于用图片撒谎

复现方式：启动后端与前端，登录后逐页截图。
可参考 `docs/images/` 中现有几张的取景范围。
