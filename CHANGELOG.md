...
**通用性清单最后一项结构性空白闭合，且框架具备交付形态。**

### Added

- **跨源实体消解**（ADR-0018）：一个对象可以跨两个数据源，聚合也可以跨源。
  客户主数据在 CRM、合同在 ERP 是 B 端常态，而此前完全无法表达。

  真正的前置问题不是连接管理，而是「CRM 第 42 行和 ERP 第 7 行是同一个客户吗」——
  那个判断会进入判定链，**若它不可核验，基于它的每个结论都不可解释**。
  解法是把匹配变成**声明**：`CRM.tax_id ↔ ERP.taxpayer_no`。
  不做相似度、不做 ML 实体链接——那些会让判定结论随阈值变化。

  三条拒绝构成可核验性：副源匹配到多行时**拒绝**而非挑第一行；
  两侧同名字段取值不同时**标记冲突**，`ConflictingValue` 拒绝参与比较，
  由 fail-closed 转为「未通过 + 原因」；匹配失败时**不注入任何字段**。
  且不做传递推导——A↔B 与 B↔C 不会自动得出 A↔C。
- **可执行一致性契约随包发布**（ADR-0016）：覆盖 5 个扩展点。
  阶段 D 原记为「阻塞于缺少第二个真实用例」，那是循环论证——
  验证一个实现需要读我们的测试套件，就不会有第二个用例。
  不依赖 pytest：要求装测试框架才能验证插件，对最需要它的集成方恰是道门槛。
- **部署前自检与私有化部署**（ADR-0017）：`aletheia preflight` 拦下静默误配置
  （`ONTOLOGY_AUTH_DISABLED` 残留会让整套 API 免鉴权暴露）。
  `aletheia serve` 在非本机监听时**强制**自检并拒绝启动。
  两阶段非 root 镜像、参考编排、`.env.example`（无任何可用默认值）。
- **问答回归测试集**：断言结构性属性而非措辞，并由自省守卫强制这一点——
  带模型的回归集会因供应商改模型而失败，随后被标记 flaky 并跳过。
- `SchemaBundle` 具备**加列**能力（`ColumnAddition`）。

### Fixed

- **检索期权限过滤**：知识条目候选集此前按本体与锚点过滤，**不按调用者的对象权限过滤**。
  于是只能读某个对象的角色，可以在答案里看到锚定在另一个对象上的条款原文。
  过滤发生在锚定之后、排序之前——排序之后再丢，
  意味着已经用不该看的文本决定了展示什么。角色取自认证身份，绝不取自请求体。
- **`SchemaBundle` 无法给已部署的库加列**：`create table if not exists`
  对已存在的表什么都不做，因此新增字段**只对全新部署生效**——
  功能在开发机上能用、升级后不能用，且不会有任何报错。
