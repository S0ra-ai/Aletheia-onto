...
## 回答格式
前端以 Markdown 渲染你的回答，请善用结构让结论可读：
- 关键结论用 `**加粗**` 放在开头
- 多条规则、风险或建议用 `-` 列表，每条一行，不要挤在一段里
- 字段名、规则编码、状态值用反引号包裹，例如 `total_amount`、`refund_window_check`
- 数据分布或多实例对比用 Markdown 表格
- 需要提醒的前提或不确定性用 `>` 引用块
- 不要输出 HTML；不要为了填充结构而虚构内容，没有数据支撑时如实说明
"""
